        ...
        with SessionLocal() as db:
            ...
            if spot:
                ...
                if prev != status:
                    ...
                    if status == 0: # Wolne -> szukamy subskrybentów
                        ...
                        for sub in subs:
                            ...
                            db.delete(sub) # Jednorazowe powiadomienie
                db.commit()
    except Exception as e:
...
# --- NOWY ENDPOINT SUBSKRYPCJI (UŻYWA DEVICE_SUBSCRIPTIONS) ---
...
def subscribe_spot(r: SubscriptionRequest, db: Session=Depends(get_db)):
    ...
    # 1. Usuń ewentualne stare wpisy dla tego tokena i miejsca (żeby nie dublować)
...
async def iot(d: dict, db: Session=Depends(get_db)):
    ...
    if s:
        ...
        if prev != status:
            ...
            if status == 0: 
                # UŻYWAMY NOWEJ TABELI SUBSKRYPCJI
                subs = db.query(DeviceSubscription).filter(DeviceSubscription.sensor_name == name).all()
                for sub in subs:
                    send_push(sub.device_token, "Wolne Miejsce!", f"{name} jest wolne.")
                    db.delete(sub)
