...
def send_push_notification(token, title, body, data):
    ...
    try: 
        payload = {"to": token, "title": title, "body": body, "data": data, "sound": "default", "priority": "high", "channelId": "parking_channel", "_displayInForeground": True}
...
def forecast(target_date: Optional[str] = None, target_hour: Optional[int] = None, db: Session = Depends(get_db)):
    # ... (Skopiuj logikę z poprzedniej wersji) ...
    return {}
@app.post("/api/v1/statystyki/zajetosc")
def stats(z: StatystykiZapytanie, db: Session = Depends(get_db)):
    key = f"stats:{z.sensor_id}:{z.selected_date}:{z.selected_hour}"; 
    if redis_client and redis_client.get(key): return {"wynik_dynamiczny": json.loads(redis_client.get(key))}
...
def rep(r: RaportRequest, request: Request, db: Session = Depends(get_db)):
    # ... (Skopiuj logikę raportu z poprzedniej wersji) ...
    return {}
...
def get_airbnb_offers(db: Session = Depends(get_db)):
    ...
    return [{
        ...
        "price": o.price, "availability": o.availability, "period": o.period,
...
def add_airbnb_offer(a: AirbnbAdd, db: Session = Depends(get_db)):
    ...
    if not user: raise HTTPException(401, "Auth error")
    new_offer = AirbnbOffer(title=a.title, description=a.description, price=a.price, availability=a.availability, period="Nieokreślony", owner_name=user.email.split('@')[0], contact=user.email, latitude=a.latitude, longitude=a.longitude)
...
def update_offer_location(u: AirbnbLocationUpdate, db: Session = Depends(get_db)):
    ...
    # Sprawdzenie właściciela (uproszczone)
...
# ... (MQTT, Threading, Startup - bez zmian) ...
mqtt_c = mqtt.Client()
def mqtt_loop():
...
def check_stale():
    while not threading.main_thread().is_alive() is False:
        try:
            db = SessionLocal(); cut = now_utc() - datetime.timedelta(minutes=5);
