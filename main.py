...
# ... (Konfiguracja MQTT i Bazy Danych bez zmian) ...
...
# ... (Definicje Tabel: AktualnyStan, DaneHistoryczne, OstatniStanBramki, ObserwowaneMiejsca bez zmian) ...
...
# ... (Funkcje: get_db, check_api_key, definicja app i CORSMiddleware bez zmian) ...
def get_db():
    db = SessionLocal()
    try:
        yield db
...
# ... (Modele Pydantic: WymaganyFormat, ObserwujRequest, StatystykiZapytanie bez zmian) ...
...
# ... (Funkcje pomocnicze: send_push_notification, get_time_with_offset, calculate_occupancy_stats bez zmian) ...
...
def calculate_occupancy_stats(sensor_prefix: str, selected_date_obj: datetime.date, selected_hour: int, db: Session) -> dict:
    ...
    for rekord in wszystkie_dane_dla_sensora:
        if not rekord.czas_pomiaru: continue
        dzien_rekordu = rekord.czas_pomiaru.weekday()
        czas_rekordu = rekord.czas_pomiaru.time()
        if dzien_rekordu not in dni_do_uwzglednienia: continue
        if czas_poczatek > czas_koniec:
            if not (czas_rekordu >= czas_poczatek or czas_rekordu < czas_koniec): continue
        else:
            if not (czas_poczatek <= czas_rekordu < czas_koniec): continue
...
# ... (Endpointy: read_root, obserwuj_miejsce, pobierz_statystyki_zajetosci bez zmian) ...
@app.get("/")
def read_root():
...
def obserwuj_miejsce(request: ObserwujRequest, db: Session = Depends(get_db)):
    ...
    if wpis:
        ...
        nowy_obserwator = ObserwowaneMiejsca(device_token=token, sensor_id=sensor_id, czas_dodania=teraz)
...
# === ❗️ ZMODYFIKOWANY ENDPOINT PROGNOZ (ODPORNY NA BŁĘDY) ===
...
# ... (Reszta pliku: process_parking_update, aktualizuj_miejsce_http, pobierz_aktualny_stan... ) ...
def process_parking_update(dane_z_bramki: WymaganyFormat, db: Session, teraz: datetime.datetime):
    ...
    nowy_rekord_historyczny = DaneHistoryczne(czas_pomiaru=teraz, sensor_id=sensor_id, status=nowy_status)
    ...
    if miejsce_db:
        ...
        nowe_miejsce = AktualnyStan(sensor_id=sensor_id, status=nowy_status, ostatnia_aktualizacja=teraz)
        db.add(nowe_miejsce)
...
# ... (Implementacja MQTT i Hooki startup/shutdown bez zmian) ...
