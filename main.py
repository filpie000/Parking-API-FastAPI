...
class Admin(Base):
    ...
    badge_name = Column(String) # np. 'Super Admin'

# 2. Aktualny Stan Czujników
...
# Model logowania dla Admina Dashboardu
...
def calculate_occupancy_stats(sensor_prefix: str, selected_date_obj: date, selected_hour: int, db: Session) -> dict:
    ...
    if nazwa_swieta:
        ...
        if wd <= 3: kategoria_str = "Dni robocze"
        elif wd == 4: kategoria_str = "Piątek"
        else: kategoria_str = "Weekend"
    ...
    for r in rows:
        ...
        if not nazwa_swieta:
            if wd <= 3 and czas_pl.weekday() > 3: continue
            if wd == 4 and czas_pl.weekday() != 4: continue
            if wd >= 5 and czas_pl.weekday() < 5: continue
...
def get_all_users(db: Session = Depends(get_db)):
    # W realnej produkcji tutaj powinna być weryfikacja, czy requester jest adminem!
...
def create_default_admins(db: Session):
    ...
    for a in admins:
        db.add(Admin(
            username=a["user"],
            password_hash=get_password_hash(a["pass"]),
            role=a["role"],
            badge_name=a["badge"]
        ))
    db.commit()
